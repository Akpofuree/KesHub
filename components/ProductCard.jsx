...
const ProductCard = ({ product }) => {
  ...
  return (
    <Link
      href={productPath}
      className="group max-xl:mx-auto block w-full max-w-[240px]"
    >
      <div className="bg-[#F5F5F5] h-40 sm:h-68 rounded-lg flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col gap-2 translate-x-12 group-hover:translate-x-0 transition-transform duration-300 opacity-0 group-hover:opacity-100 z-10">
          ...
          <button
            ...
            className="bg-white p-2 rounded-full shadow-md text-slate-600 hover:text-green-600 hover:bg-slate-50 transition-colors disabled:opacity-50 active:scale-90"
            title="Add to Wishlist"
            onClick={handleAddToWishlist}
          >
            <HeartIcon
              size={18}
              className={isAddingToWishlist ? "animate-pulse" : ""}
            />
          </button>
        </div>
      </div>
