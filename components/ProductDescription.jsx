    ...
    return (
        <div className="my-18 text-sm text-slate-600">
                ...
                <div className="flex flex-col gap-3 mt-14">
                    {ratings.length === 0 ? (
                        ...
                        <div key={index} className="flex gap-5 mb-10">
                            <Image src={item.user?.image || "/favicon.png"} alt="" className="size-10 rounded-full" width={100} height={100} />
                            <div>
            ...
            <div className="flex gap-3 mt-14">
                <Image src={product.store?.logo || "/favicon.png"} alt="" className="size-11 rounded-full ring ring-slate-400" width={100} height={100} />
                <div>
                    <p className="font-medium text-slate-600">Product by {product.store?.name || "KES HUB"}</p>
                    {product.store?.username ? (
                        <Link href={`/storefront/${product.store.username}`} className="flex items-center gap-1.5 text-green-500"> view store <ArrowRight size={14} /></Link>
                    ) : null}
                </div>
            </div>
        </div>
