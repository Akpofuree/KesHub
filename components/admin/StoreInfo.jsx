...
const StoreInfo = ({store}) => {
    return (
        <div className="flex-1 space-y-2 text-sm">
            <FallbackImage width={100} height={100} src={store.logo} alt={store.name} className="max-w-20 max-h-20 object-contain shadow rounded-full max-sm:mx-auto" fallbackSrc={assets.gs_logo} />
            ...
            <div className="flex items-center gap-2 text-sm ">
                <FallbackImage width={36} height={36} src={store.user.image} alt={store.user.name} className="w-9 h-9 rounded-full" fallbackSrc={assets.gs_logo} />
                <div>
